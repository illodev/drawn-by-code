...
(() => {
    ...
    // ---------------------------------------------------------------- DESK (top-down)
    Sets.desk = (g, t, env, o = {}) => {
        WL.sprite('desktop', { x: -200, y: -200, w: 1400, h: 1400 }, (c) => {
            c.fillStyle = C.wood;
            c.fillRect(-200, -200, 1400, 1400);
            ...
            P.cutout(c, P.ellipse(330, 260, 420, 420), C.light, 'light', { border: 0, shadow: 0, jag: 1.4, tex: { alpha: [0.2, 0.4] } });
            // crayons
    ...
    // notepad with purple cover and spiral; the sheet is written with o.p
    Sets.notepad = (g, t, x, y, s, rot, o = {}) => {
        g.save();
        g.translate(x, y);
        g.rotate(rot);
        g.scale(s, s);
        ...
        for (let i = 0; i < 17; i++) {
            const xx = -270 + i * 34;
            g.beginPath();
            g.ellipse(xx, -196, 6, 16, 0, Math.PI * 1.1, Math.PI * 2.4);
            g.stroke();
        }
        g.restore();
    };

    // arm entering from the bottom right to the hand at (hx, hy), holding a pencil
