// Segment 4 · The climax · kaleidoscope (18–23.5 s)
// Opens from the point where the vortex ended. The pieces are bits of the previous
// segments (frog eyes, mushroom caps, blobs, marbles) coming out of the center.
// Hue change on the 20.5 beat.
(() => {
    const E = Ease;
    const INK = Groovy.PAL.acid.ink, C = Groovy.PAL.acid.c;
    ...
    Segment.kaleido = (g, t, env) => {
        ...
        Kaleido.draw(g, env, {
            ...
            source: (s) => {
                // wedge background: color bands coming out of the center,
                // from largest to smallest, or the small ones get covered
        ...
        // the hub: the marble in the center, big, unmirrored
        Kaleido.beads(g, 800, 450, 88 * open, 16, 9 * open, lt * 1.5, [C[1], C[4], C[5], C[2]]);
        // it never disappears: at 18.0 it is the point everything is born from
        Marble.draw(g, 800, 450, Math.max(10, 52 * open) * (1 + beat * 0.08), t, 'poster');
    };
})();
