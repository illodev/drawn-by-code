...
#   python3 overlay.py 0.5,2.8,6 [--render] [--out out/overlay.jpg] [--crop y0,y1]
import sys, subprocess, numpy as np, os
from PIL import Image
args=sys.argv[1:]
times=[float(x) for x in args[0].split(',')]
render='--render' in args
out=args[args.index('--out')+1] if '--out' in args else 'out/overlay.jpg'
y0,y1=(int(v) for v in args[args.index('--crop')+1].split(',')) if '--crop' in args else (250,1000)
HERE=os.path.dirname(os.path.abspath(__file__))
if render:
    subprocess.run(['node','engine/render.mjs','sandbox/2026-09-25-felt-cats/check.js','--at',args[0],'--size','576','--out','sandbox/2026-09-25-felt-cats/out/check'],cwd=HERE+'/../..',check=True,capture_output=True)
...
for t in times:
    ...
    ours=np.asarray(Image.open(HERE+f'/out/check/t_{t:.2f}s.png').convert('RGBA')).astype(np.float32)
    a=ours[...,3:4]/255
