// 2026-09-24-coffee-first · paper-cutout style
// A sleeping mug falls onto the table, its steam wakes it up and «Primero, café.» is written.
Motion.scene({
    fps: 24,
    duration: 6,
    logical: [1600, 900],
    ...
    bpm: 120,
    shots: [[0, 2, 'Table'], [2, 4, 'Steam'], [4, 6, 'Phrase']],
    audio: { mix: 'mix.wav' },
    ...
    draw(g, t, env) {
        ...
        // camera: in «Phrase» it pans to make room for the card
        ...
        // --- the mug: falls on the 0.5 s beat, squashes and bounces -------------------
        ...
        mug(g, MUG);
        face(g, t);
        // steam: born at the coffee surface. Painted IN FRONT of the mug, clipped to
        // «above the coffee line» ∪ «inside the coffee», so the base of each strip
        // stays hidden under the surface and seems to come out of it.
        ...
        // ---------------------------------------------------------------------------
        function mug(g, M) {
            // handle: half ring on the right, behind the body
        ...
        function eye(g, x, yy, open, seed) {
            ...
            g.restore();
            if (open < 0.08) {
                // closed: the eyelid covers ALL the white and the curved lash goes on top;
                // if white peeks out under the stroke it reads as «looking down», not «asleep»
            ...
            g.fill();
            // thick eyelid coming down from above (always a bit droopy: sidelong look)
        ...
        function steam(g, t, i, x0, y0) {
            // Tissue-paper wisps (kit.wisp): born thin in the coffee, they rise, drift
            // right as in a gentle current, widen and fade out.
            // Each wisp is a fixed cutout that only moves: it does not boil.
