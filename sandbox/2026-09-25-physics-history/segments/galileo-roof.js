...
(() => {
    ...
    function armAndHand(press, R, M, bf, which, s, pole, ps) {
        ...
        const Wl = GalHands.wristPt(which === 'near' ? 'fist' : which, r), W = F.apply(Wl);
        ...
        // the near sleeve ends past the cuff; the far wrist is behind the tube, its sleeve and
        // cuff drawn before the tube so the tube hides the wrist
        const cu = far ? W : F.apply([Wl[0] + fa[0] * 27, Wl[1] + fa[1] * 27]);
        return {
            sleeve: () => {
            ...
            hand: () => {
                ...
                GalHands[which === 'near' ? 'fist' : which](press, r, { fa, squeeze: which === 'near' ? 1.5 * Math.max(0, Math.sin((ps.lean + ps.look) * Math.PI)) : 0 });
                press.restore();
            },
        };
    }
    function galileo(press, t, R, part) {
        ...
        const farA = armAndHand(press, R, M, bf, 'far', L(HAND.far, 195, ps.look), [40, 330], ps);
        if (part === 'back') {
            farA.sleeve();
            withPlane(press, M, () => {
                gown(press, bf);
