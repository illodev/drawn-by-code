// 2026-09-25-physics-newton-faraday · style risograph
// The decisive test of «A history of physics in 40 seconds» (shots FIS-03 and FIS-04, 8–20 s
// of the piece): Newton drops a ball, the fall becomes an orbit round a schematic Earth, the
// orbit winds into a copper coil, and Faraday's magnet makes a galvanometer answer going in,
// nothing while held, the other way coming out. No text. Printed per drawing (on twos).
const DIR = 'sandbox/2026-09-25-physics-newton-faraday/';
...
Motion.scene({
    ...
    setup(env) {
        ...
        return { press: Riso.press(env), sim: (t) => Math.max(-GALV.max, Math.min(GALV.max, th[Math.min(n - 1, Math.max(0, Math.round(t / dt)))])) };
    },
    draw(g, t, env) {
        const { press, sim } = env.state;
        const d = Math.floor(t * 12 + 1e-6), tq = d / 12;
        press.begin(d);
        // the night-blue ground, a little lighter behind the action
        ...
        if (pan > 0) faradayShot(press, tq, d, pan, sim);
...
function spotScale(press, tq, sim) {
    ...
    if (dark > 0) Ph.put(press, Ph.circle(800, y, dark), { blue: 0.9, 'navy.s': 0.9 });
    ...
    if (tq >= TM.spot[0] + 0.25) {
        ...
        Ph.put(press, (g) => g.rect(-100, top, 1800, h), { 'yellow.s': 0.16, 'pink.s': 0.06 });
        Ph.ink(press, (g) => g.rect(-100, top + h - 12, 1800, 12), { 'navy.s': 0.35, 'pink.s': 0.2 });
        for (let i = -30; i <= 30; i++) {
