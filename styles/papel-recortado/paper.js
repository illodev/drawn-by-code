// Kit de papel recortado para canvas 2D (sacado de «La caja y el torno», de Fube).
// Todo es determinista: la misma semilla da el mismo rasgado y la misma textura en cada
// fotograma, así que nada "hierve". Depende de engine/core.js (Motion, Ease).
const Paper = (() => {
    const PAPER = '#f7f1e3';
    ...
    // Ruido 1D periódico en [0,1): sirve para variar el grosor del borde a lo largo del contorno.
    ...
    // Contorno de una unión de círculos vista desde (0,0): vale para formas
    // "estrelladas" como una nube, y deja muescas afiladas como un corte de tijera.
    ...
    // Tira de grosor w a lo largo de una polilínea, con remates redondos.
    ...
    // Desplaza el contorno por su normal: `offset` de fondo (con variación lenta)
    // más un dentado fino que es lo que se lee como "rasgado".
    function torn(res, seed, offset, offVar, jag) {
        ...
        return res.pts.map((p, i) => {
            ...
            if (r() < 0.05) o -= r() * jag * 2.2; // mordisco ocasional
            return [p[0] + res.nrm[i][0] * o, p[1] + res.nrm[i][1] * o];
        });
    }

    // --- textura de rotulador ------------------------------------------------------
    // Trazos cortos, planos y semitransparentes del mismo tono con la luz algo movida.
    function marker(ctx, box, color, seed, o = {}) {
        ...
        for (let i = 0; i < count; i++) {
            let x, y, a;
            if (path) {
                // repartidos a lo largo de una tira: siguen su dirección y se mueven con ella
    ...
    // Pelitos de fibra que asoman del borde blanco.
    ...
    /**
     * Pieza recortada: borde de papel blanco rasgado + capa de color con textura.
     * o.border: ancho del filo blanco (0 = sin filo); o.tex: opciones de marker();
     * o.shadow: sombra de papel pegado; o.inner(ctx): dibujo extra recortado a la pieza.
     */
    ...
    // Trazo de rotulador (líneas, bocas, subrayados): varias pasadas algo desplazadas.
    ...
    // Garabato que imita letra manuscrita (el "texto" de los papeles).
    ...
    // Grano de papel: un tile de ruido que se multiplica por encima de todo.
    ...
    // Pinta con drawFn en un lienzo aparte, a resolución de salida, para reutilizarlo.
